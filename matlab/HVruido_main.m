...
rutaarch = '/Users/antonio/Dropbox/Geofisica/Tesis/Cynthia/HVruido/estaciones/';
rutagrab = '/Users/antonio/Dropbox/Geofisica/Tesis/Cynthia/HVruido';
...
tiempoHV = [10*60];      % Tiempo (minutos) para cálculo de cada H/V (Tiempo de registro manipulable)
...
for aleat = 1 %:10
    ...
    for ee = 1:length(buscar)
        ...
        for i = 1:length(listreg)
            listdias00{i,1} = listreg{i}(1:16);
        end
        listdias0{ee} = unique(listdias00);
        nombcomun = '_TOME__1_HH_ZNE0';
        
        buscardia = listdias0{ee};
        % buscardia = {'2023185'};
        [~,Nbuscardia{ee}] = ismember(buscardia,listdias0{ee});
        for dd = 1:length(Nbuscardia{ee})
            k = Nbuscardia{ee}(dd);
            listdias = listdias0{ee};
            ...
            cont0 = 0;
            vecfechahms = [];
            for i = 1:length(listdias)
                cont0 = cont0+1;
                EWsac = [rutaarch,estac,'/',listdias{i},nombcomun,'3.sac'];
                NSsac = [rutaarch,estac,'/',listdias{i},nombcomun,'2.sac'];
                VEsac = [rutaarch,estac,'/',listdias{i},nombcomun,'1.sac'];
                [EW0,NS0,VE,dt,fechahms,w1,w2] = F_dat2mat(EWsac,NSsac,VEsac);
                vecfechahms = [vecfechahms;{fechahms}];
            end
            dt = dt(1);
            fmax = 1/(2*dt);
            [Nn,Nhoras] = size(EW0);
            ...
            for Nteta = 1:length(tetarot)
                ...
                costeta = cosd(teta);
                sinteta = sind(teta);
                EW = EW0*costeta+NS0*sinteta;  %longitudinal
                NS =-EW0*sinteta+NS0*costeta;  %transversal
...
%                         set(lg,'location','northwest','fontname','Times New Roman','fontSize',14)
                        ...
                        for hh = 1:length(horario)
                            if horario(hh) == 1; datos = [senhal,'Dia']; end
                            if horario(hh) == 2; datos = [senhal,'Noche']; end
                            if horario(hh) == 3; datos = [senhal,'DiaNoche']; end
                            
                            % SELECCIÓN HORARIO
                            horaselec = 1:length(vecfechahms);
                            if horario(hh) == 1
                                horaselec = [];
                                cont5 = 0;
                                for i = 1:length(vecfechahms)
                                    hora = str2num(vecfechahms{i}(10:11));
                                    % if hora >= 7 && hora <= 24         %--> MXN
                                    if (hora >= 12 && hora <= 24) || ...
                                            (hora >= 0 && hora <= 4)     %--> GMT
                                        cont5 = cont5+1;
                                        horaselec(cont5) = i;
                            ...
                            if horario(hh) == 2
                                ...
                                for i = 1:length(vecfechahms)
                                    hora = str2num(vecfechahms{i}(10:11));
                                    % if hora >= 0 && hora < 7     %--> MXN
                                    if hora >= 5 && hora < 12      %--> GMT
                                        cont5 = cont5+1;
                                        horaselec(cont5) = i;
                                    end
                                end
                            end
                            Nhoras2 = length(horaselec);
                            Nvent = sum(sum(wincleantot(:,horaselec)));
                            
                            %%%%% EVALUACIÓN %%%%%
                            if isempty(horaselec)
                                continue
                            end
                            %%%%%%%%%%%%%%%%%%%%%%
                            
                            % SELECCIÓN VENTANAS SEGÚN EL HORARIO
                            Nvini = Nhoras2*M; %Nvini = M;
                            ...
                            for j = 1:Nhoras2
                                for kk = 1:M
                                    cont = cont+1;
                                    EWv(:,cont) = EW(iv(kk):fv(kk),horaselec(j));
                                    NSv(:,cont) = NS(iv(kk):fv(kk),horaselec(j));
                                    VEv(:,cont) = VE(iv(kk):fv(kk),horaselec(j));
                                    fechahmsvent(cont,1) = {[vecfechahms{horaselec(j)},'_',num2str(kk)]};
                                end
                            end
                            wincleantotrev = wincleantot(:,horaselec);
                            wincleantotlinea = reshape(wincleantot(:,horaselec),[1,Nvini]);
                            ventok = find(wincleantotlinea~=0);
                            cumsumwinclean = cumsum(wincleantotlinea);
...
%                             % cla
                            ...
                            for norm = 1:length(normalizac(:,1))                                
                                ...
                                nombcomb = [datos,'-',unidad,'-','filt',num2str(w1),'to',num2str(w2), ...
                                ...
                                nombcomb = strrep(nombcomb,'noiseDia-','D-');
                                nombcomb = strrep(nombcomb,'noiseNoche-','N-');
                                nombcomb = strrep(nombcomb,'noiseDiaNoche-','DN-');
