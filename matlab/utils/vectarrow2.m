function vectarrow2(p0,p1,alpha,beta,co,lw)
...
if max(size(p0))==3
    if max(size(p1))==3
        ...
        y1 = p1(2);
        z1 = p1(3);
        plot3([x0;x1],[y0;y1],[z0;z1],'color',co,'linewidth',lw); hold on  % Draw a line between p0 and p1
        
        p = p1-p0;
        % if sum(p(1:2)) == 0; eps = 0.3; end
        hu = [x1-alpha*(p(1)+beta*(p(2)+eps)); x1; x1-alpha*(p(1)-beta*(p(2)+eps))];
        hv = [y1-alpha*(p(2)-beta*(p(1)+eps)); y1; y1-alpha*(p(2)+beta*(p(1)+eps))];
        hw = [z1-alpha*p(3);z1;z1-alpha*p(3)];
        
        hold on
        plot3(hu(:),hv(:),hw(:),'color',co,'linewidth',lw); hold on  % Plot arrow head
        % grid on
        %xlabel('x')
        %ylabel('y')
        %zlabel('z')
...
elseif max(size(p0))==2
    if max(size(p1))==2
        ...
        x1 = p1(1);
        y1 = p1(2);
        plot([x0;x1],[y0;y1],'color',co,'linewidth',lw); hold on   % Draw a line between p0 and p1
        ...
        hold on
        plot(hu(:),hv(:),'color',co,'linewidth',lw); hold on  % Plot arrow head
        % grid on
        %xlabel('x')
        %ylabel('y')
        hold off
